...
class WalletSmsTemplate {
  ...
  factory WalletSmsTemplate.fromStrings({
    ...
    return WalletSmsTemplate(
      ...
      incomePatterns: incomePatterns
          .map((p) => RegExp(p, caseSensitive: false, dotAll: true))
          .toList(),
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'senderIds': senderIds,
      'incomePatterns':
          incomePatternStrings ?? incomePatterns.map((r) => r.pattern).toList(),
      'expensePatterns':
          expensePatternStrings ??
          expensePatterns.map((r) => r.pattern).toList(),
      'balancePatterns':
          balancePatternStrings ??
          balancePatterns.map((r) => r.pattern).toList(),
    };
  }
...
class SmsSenderRegistry {
  ...
  static void registerCustomTemplate(
  ...
  static void registerCustomTemplates(
    List<WalletSmsTemplate> templates, {
    bool persist = false,
  }) {
  ...
  static List<RegExp> get standardIncomePatterns => [
    ...
    RegExp(
      r'(?:تم\s*تحويل|تحويل)\s+(?:ب?مبلغ\s+|ب?قيمة\s+)?(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*([0-9,]+(?:\.[0-9]+)?)\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*(?:إلى\s*محفظتك|الى\s*محفظتك|لحساب(?:ك|كم)|إلى\s*حساب(?:ك|كم)|الى\s*حساب(?:ك|كم)|في\s*حساب(?:ك|كم)|لك\b)',
      caseSensitive: false,
    ),

    // 2. Explicit amount with "مبلغ" or "بقيمة" following any deposit trigger (safely skips phone numbers/account numbers)
    RegExp(
      r'(?:[أا]ودع(?:ت)?|إيداع|ايداع|الإيداع|الايداع|تمت?\s*[إا]يداع|إيداع\s*نقدي|ايداع\s*نقدي|أضيف(?:ت)?|اضيف(?:ت)?|تمت?\s*[إا]ضاف[ةه]|[إا]ضاف[ةه]|تمت?\s*تغذي[ةه]|تغذي[ةه]|تحويل\s*وارد|حوال[ةه]\s*وارد[ةه]|حوال[ةه]\s*من|تحويل\s*من|استلام\s*(?:حوال[ةه]|تحويل|مبلغ)?|تم\s*استلام|استلمت|وصلك(?:ت)?\s*(?:تحويل|حوال[ةه]|مبلغ)?|قيد\s*(?:ل|في)?حساب(?:ك|كم)|تم\s*(?:قيد|القيد)|شحن|قبض|توريد|تم\s*توريد|دائن|إشعار\s*دائن|اشعار\s*دائن|عكس\s*قيد|لصالحك|قيد\s*لصالح(?:ك|كم)|حساب(?:ك|كم)\s*دائن)[\s\S]*?(?:ب?مبلغ|ب?قيمة)\s*[:\s]*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*([0-9,]+(?:\.[0-9]+)?)',
      caseSensitive: false,
      dotAll: true,
    ),
    ...
    RegExp(
      r'([0-9,]+(?:\.[0-9]+)?)\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)\s*(?:إلى\s*محفظتك|الى\s*محفظتك|لحساب(?:ك|كم)|إلى\s*حساب(?:ك|كم)|الى\s*حساب(?:ك|كم)|في\s*حساب(?:ك|كم)|[أا]ودع|إيداع|ايداع|أضيف|اضيف|[إا]ضاف[ةه]|استلام|تغذي[ةه]|وارد|دائن|لصالحك|تحويل\s*مشترك)',
      caseSensitive: false,
    ),

    // 4. Keyword followed by currency and amount (e.g. اضيف 500ر.ي or إيداع 1000 YER or تغذية 20000 ريال)
    RegExp(
      r'(?:أضيف|اضيف|أضيفت|اضيفت|إيداع|ايداع|تغذية|تغذيه|استلام|استلمت|دائن|عكس\s*قيد|لصالحك)\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*([0-9,]+(?:\.[0-9]+)?)\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)',
      caseSensitive: false,
    ),

    // 5. Direct keyword followed immediately by amount (excluding 9-digit Yemeni phone numbers starting with 7)
    RegExp(
      r'(?:[أا]ودع(?:ت)?(?:\/[^\d\n]+|\s+[^\d\n]+)?(?:\s+(?:ل|في|إلى|الى|ب)?حساب(?:ك|كم)|\s+لك)?|إيداع|ايداع|الإيداع|الايداع|تمت?\s*[إا]يداع|أضيف(?:ت)?|اضيف(?:ت)?|تمت?\s*[إا]ضاف[ةه]|تغذي[ةه]|تم\s*استلام|استلام|لحساب(?:ك|كم)|إلى\s*محفظتك|الى\s*محفظتك|دائن|عكس\s*قيد|لصالحك)[\s:]*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*(?!(?:7[01378]\d{7}))([0-9,]+(?:\.[0-9]+)?)',
      caseSensitive: false,
    ),
  ];
  ...
  static final List<WalletSmsTemplate> defaultTemplates = [
    ...
    WalletSmsTemplate(
      ...
      expensePatterns: [
        RegExp(
          r'(?:شراء|مشتريات|مشترياتك|حاسب|خدمة\s*حاسب|نقاط\s*البيع|نقطة\s*بيع|pos|دفع|تم\s*دفع|خصم|تم\s*خصم|قيد\s*خصم|سداد|تم\s*سداد|(?:تم\s*تحويل|تحويل)(?!\s*وارد)(?![\s\S]*?(?:لحساب(?:ك|كم)|إلى\s*حساب(?:ك|كم)|الى\s*حساب(?:ك|كم)|في\s*حساب(?:ك|كم)|إلى\s*محفظتك|الى\s*محفظتك|لك\b))|سحب|تم\s*سحب|حوال[ةه]\s*صادر[ةه])[\s\S]*?(?:(?:ب?مبلغ|ب?قيمة)\s*)?(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*(?!(?:7[01378]\d{7}))([0-9,]+(?:\.[0-9]+)?)',
          caseSensitive: false,
          dotAll: true,
        ),
        RegExp(
          r'([0-9,]+(?:\.[0-9]+)?)\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)\s*(?:خصم|شراء|سداد|(?:تحويل)(?!\s*وارد)(?![\\s\\S]*?(?:لحساب(?:ك|كم)|إلى\s*حساب(?:ك|كم)|الى\s*حساب(?:ك|كم)|في\s*حساب(?:ك|كم)|إلى\s*محفظتك|الى\s*محفظتك|لك\b))|سحب|حاسب)',
          caseSensitive: false,
        ),
      ],
      balancePatterns: [
        RegExp(
          r'(?:الرصيد|رصيدك|رصيد)\s*(?:حسابك)?\s*(?:الحالي|المتاح|المتبقي|الفعلي|هو)?[:\s]*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*([0-9,]+(?:\.[0-9]+)?)',
          caseSensitive: false,
        ),
        RegExp(
          r'([0-9,]+(?:\.[0-9]+)?)\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)\s*(?:الرصيد|رصيدك)',
          caseSensitive: false,
        ),
        RegExp(
          r'(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)\s*([0-9,]+(?:\.[0-9]+)?)\s*(?:الرصيد|رصيدك)',
          caseSensitive: false,
        ),
        RegExp(
          r'رصيد\s*(?:حسابك)?(?:\s+في\s+[^\d]+)?\s*(?:هو|:)?\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*([0-9,]+(?:\.[0-9]+)?)',
          caseSensitive: false,
        ),
      ],
    ),
    ...
    WalletSmsTemplate(
      ...
      expensePatterns: [
        RegExp(
          r'(?:تمت?\s*عملية\s*خصم|تم\s*خصم|خصم|تم\s*سداد|سداد|تم\s*دفع|دفع|(?:تم\s*تحويل|تحويل)(?!\s*وارد)(?![\s\S]*?(?:لحساب(?:ك|كم)|إلى\s*حساب(?:ك|كم)|الى\s*حساب(?:ك|كم)|في\s*حساب(?:ك|كم)|إلى\s*محفظتك|الى\s*محفظتك|لك\b))|حوال[ةه]\s*صادر[ةه]|تم\s*شراء|شراء|مشتريات|تم\s*سحب|سحب|قيد\s*خصم)[\s\S]*?(?:(?:ب?مبلغ|ب?قيمة)\s*)?(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*(?!(?:7[01378]\d{7}))([0-9,]+(?:\.[0-9]+)?)',
          caseSensitive: false,
          dotAll: true,
        ),
        RegExp(
          r'([0-9,]+(?:\.[0-9]+)?)\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)\s*(?:خصم|سداد|تحويل|شراء|سحب)',
          caseSensitive: false,
        ),
      ],
      balancePatterns: [
        RegExp(
          r'رص:\s*([0-9,]+(?:\.[0-9]+)?)\s*(?:ر\.?ي|YER|ريال)?',
          caseSensitive: false,
        ),
        RegExp(
          r'(?:الرصيد|رصيدك|رصيد)\s*(?:حسابك)?\s*(?:الحالي|المتاح|المتبقي|الفعلي|هو)?[:\s]*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*([0-9,]+(?:\.[0-9]+)?)',
    ...
    WalletSmsTemplate(
      ...
      expensePatterns: [
        RegExp(
          r'(?:خصم|تم\s*خصم|تم\s*سداد|سداد|سحب|تم\s*سحب|تم\s*شراء|شراء|مشتريات|دفع|تم\s*دفع|(?:تم\s*تحويل|تحويل)(?!\s*وارد)(?![\\s\\S]*?(?:لحساب(?:ك|كم)|إلى\s*حساب(?:ك|كم)|الى\s*حساب(?:ك|كم)|في\s*حساب(?:ك|كم)|إلى\s*محفظتك|الى\s*محفظتك|لك\b)))[\s\S]*?(?:(?:ب?مبلغ|ب?قيمة)\s*)?(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*(?!(?:7[01378]\d{7}))([0-9,]+(?:\.[0-9]+)?)',
          caseSensitive: false,
          dotAll: true,
        ),
      ],
      balancePatterns: [
        RegExp(
          r'(?:الرصيد|رصيدك|رصيد)\s*(?:الحالي|هو)?[:\s]*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*([0-9,]+(?:\.[0-9]+)?)',
          caseSensitive: false,
        ),
        RegExp(
          r'رصيد\s*(?:حسابك)?(?:\s+في\s+[^\d]+)?\s*(?:هو|:)?\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*([0-9,]+(?:\.[0-9]+)?)',
        ...
        RegExp(
          r'([0-9,]+(?:\.[0-9]+)?)\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)\s*رصيدك',
          caseSensitive: false,
        ),
      ],
    ),
    ...
    WalletSmsTemplate(
      ...
      expensePatterns: [
        RegExp(
          r'(?:تم\s*خصم|خصم|تم\s*سداد|سداد|شراء|مشتريات|تم\s*شراء|دفع|تم\s*دفع|سحب|(?:تم\s*تحويل|تحويل)(?!\s*وارد)(?![\\s\\S]*?(?:لحساب(?:ك|كم)|إلى\s*حساب(?:ك|كم)|الى\s*حساب(?:ك|كم)|في\s*حساب(?:ك|كم)|إلى\s*محفظتك|الى\s*محفظتك|لك\b)))[\s\S]*?(?:(?:ب?مبلغ|ب?قيمة)\s*)?(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*(?!(?:7[01378]\d{7}))([0-9,]+(?:\.[0-9]+)?)',
          caseSensitive: false,
          dotAll: true,
        ),
      ],
      balancePatterns: [
        RegExp(
          r'(?:الرصيد|رصيدك|رصيد)\s*(?:الحالي|المتاح|هو)?[:\s]*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*([0-9,]+(?:\.[0-9]+)?)',
          caseSensitive: false,
        ),
        RegExp(
          r'رصيد\s*(?:حسابك)?(?:\s+في\s+[^\d]+)?\s*(?:هو|:)?\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*([0-9,]+(?:\.[0-9]+)?)',
    ...
    WalletSmsTemplate(
      ...
      expensePatterns: [
        RegExp(
          r'(?:تم\s*خصم|خصم|تم\s*سداد|سداد فاتورة|سداد|سحب|شراء|مشتريات|تم\s*شراء|دفع|تم\s*دفع|(?:تم\s*تحويل|تحويل)(?!\s*وارد)(?![\\s\\S]*?(?:لحساب(?:ك|كم)|إلى\s*حساب(?:ك|كم)|الى\s*حساب(?:ك|كم)|في\s*حساب(?:ك|كم)|إلى\s*محفظتك|الى\s*محفظتك|لك\b)))[\s\S]*?(?:(?:ب?مبلغ|ب?قيمة)\s*)?(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*(?!(?:7[01378]\d{7}))([0-9,]+(?:\.[0-9]+)?)',
          caseSensitive: false,
          dotAll: true,
        ),
      ],
    ...
    WalletSmsTemplate(
      ...
      balancePatterns: [
        RegExp(
          r'(?:الرصيد|رصيدك|رصيد)\s*(?:الحالي|المتاح|المتبقي|الفعلي|هو)?[:\s]*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*([0-9,]+(?:\.[0-9]+)?)',
          caseSensitive: false,
        ),
        RegExp(
          r'رصيد\s*(?:حسابك)?(?:\s+في\s+[^\d]+)?\s*(?:هو|:)?\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*([0-9,]+(?:\.[0-9]+)?)',
    ...
    WalletSmsTemplate(
      ...
      expensePatterns: [
        RegExp(
          r'(?:خصم|تم\s*خصم|قيد\s*خصم|سداد|تم\s*سداد|دفع|تم\s*دفع|(?:تم\s*تحويل|تحويل)(?!\s*وارد)(?![\\s\\S]*?(?:لحساب(?:ك|كم)|إلى\s*حساب(?:ك|كم)|الى\s*حساب(?:ك|كم)|في\s*حساب(?:ك|كم)|إلى\s*محفظتك|الى\s*محفظتك|لك\b))|حوال[ةه]\s*صادر[ةه]|شراء|مشتريات|تم\s*شراء|سحب|تم\s*سحب)[\s\S]*?(?:(?:ب?مبلغ|ب?قيمة)\s*)?(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*(?!(?:7[01378]\d{7}))([0-9,]+(?:\.[0-9]+)?)',
          caseSensitive: false,
          dotAll: true,
        ),
        RegExp(
  ...
  static bool isBlockedSender(String sender) {
    final normalized = sender.toLowerCase().replaceAll(
      RegExp(r'[^a-z0-9]'),
      '',
    );
    return normalized == 'mfloos';
  }
  ...
  static WalletSmsTemplate? findTemplate(
    ...
    if (isBlockedSender(cleanSender)) return null;
  ...
  static String? extractReferenceNumber(String body) {
    final patterns = [
      RegExp(
        r'(?:المرجع|مرجع|ref(?:erence)?|ref\s*#|trx[:\s]|txn[:\s]|trans(?:\s*id)?[:\s]|رقم\s*العملية|رقم\s*المرجع|رقم\s*الحوالة|رقم\s*[إا]شعار|رقم\s*ال[إا]شعار|ال[إا]شعار|[إا]شعار\s*رقم|رقم\s*السند|رقم\s*الحركة|عملية\s*رقم|حركة\s*رقم|رقم\s*القيد|كود\s*الحركة)[\s:#]*([0-9a-zA-Z]+)',
        caseSensitive: false,
      ),
    ];
  ...
  static String generateSmsKey(String sender, DateTime date, String body) {
    ...
    final referenceNumber = extractReferenceNumber(body);
    final isPurchase = [
      'شراء',
      'مشتريات',
      'حاسب',
    ...
    if (isPurchase && referenceNumber != null) {
      return '${cleanSender}_ref_${referenceNumber}_$bodyHash';
    }

    final timestampSeconds = date.millisecondsSinceEpoch ~/ 1000;
    return '${cleanSender}_${timestampSeconds}_$bodyHash';
  }
  ...
  static ParsedSmsData? parseMessage({
    ...
    if (amount == null || type == null) {
      final isInc =
          textWithoutBalance.contains('أودع') ||
      ...
      final isExp =
          textWithoutBalance.contains('خصم') ||
      ...
      if (isInc || isExp) {
        final fallbackPatterns = [
          ...
          RegExp(
            r'(?:تم\s+)?(?:تحويل|توريد|إيداع|ايداع)\s+(?:مبلغ\s+)?([0-9,]+(?:\.[0-9]+)?)\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*(?:إلى|الى|في|لحساب)',
            caseSensitive: false,
          ),
          // 2. Explicit amount keyword
          RegExp(
            r'(?:ب?مبلغ|ب?قيمة)\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?\s*([0-9,]+(?:\.[0-9]+)?)\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)?',
            caseSensitive: false,
          ),
          // 3. Currency adjacent
          RegExp(
            r'(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)\s*([0-9,]+(?:\.[0-9]+)?)',
            caseSensitive: false,
          ),
          RegExp(
            r'([0-9,]+(?:\.[0-9]+)?)\s*(?:YER|SAR|USD|ر\.?ي|ر\.?س|ريال|\$)',
            caseSensitive: false,
          ),
          // 4. Any number excluding 9-digit Yemeni phone numbers
          RegExp(
            r'(?<!\d)(?!(?:7[01378]\d{7}))([0-9]{1,3}(?:,[0-9]{3})*(?:\.[0-9]+)?|[0-9]+(?:\.[0-9]+)?)(?!\d)',
            caseSensitive: false,
          ),
        ];
    ...
    final hasTransactionAction = isPromo
        ? false
