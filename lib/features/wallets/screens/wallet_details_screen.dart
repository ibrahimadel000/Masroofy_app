  ...
  void _showDeleteConfirmation(BuildContext context, Wallet wallet) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'هل أنت متأكد من حذف محفظة "${wallet.name}"؟\nسيتم حذف المحفظة وجميع حركاتها محلياً ومن السحابة، ولا يمكن التراجع.',
        ),
        actions: [
          TextButton(
              ...
              if (newBalance != null) {
                ...
                if (delta != 0) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        'تمت تسوية الرصيد بفارق: ${AppConstants.formatCurrency(delta)}',
                      ),
                      backgroundColor: AppTheme.primaryColor,
                    ),
                  );
      ...
      builder: (context, walletsState) {
        ...
        return BlocBuilder<TransactionsCubit, TransactionsState>(
          builder: (context, txState) {
            final allTx = txState is TransactionsLoaded
                ? txState.transactions
                : <TransactionModel>[];
            final walletTransactions = allTx
